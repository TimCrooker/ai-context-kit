...
describe("buildAll with skills", () => {
  ...
  it("creates skill mirrors during build", () => {
    buildAll(tmp);
    expect(
      fs.lstatSync(path.join(tmp, ".agents/skills/demo")).isSymbolicLink(),
    ).toBe(true);
    expect(
      fs.lstatSync(path.join(tmp, ".claude/skills/demo")).isSymbolicLink(),
    ).toBe(true);
  });
  ...
  it("emits only the whitelisted agent's mirror for agents: [claude]", () => {
    fs.writeFileSync(
      path.join(tmp, ".ai/skills/demo/SKILL.md"),
      "---\nname: demo\ndescription: A demo\nagents: [claude]\n---\n\n# Demo\n",
    );
  ...
  it("removes a mirror excluded by a newly-added agent filter with --remove-orphans", () => {
    buildAll(tmp);
  ...
  it("--check fails when source exists but mirror is missing", () => {
    ...
    fs.rmSync(path.join(tmp, ".agents/skills/demo"), {
      recursive: true,
      force: true,
    });
