...
function parseJsonFile<T>(filePath: string): T {
  try {
    ...
    throw new ContextError("AICTX_CONFIG_INVALID", `Invalid JSON in ${toPosix(filePath)}`, {
...
function asString(value: unknown, msg: string): string {
  if (typeof value !== "string" || !value.trim()) {
    throw new ContextError("AICTX_CONFIG_INVALID", msg);
  }
  return value.trim();
}
...
export function loadManifest(cwd: string, manifestPath?: string): Manifest {
  ...
  if (!exists(resolved)) {
    throw new ContextError("AICTX_CONFIG_MISSING", `Missing manifest: ${rel(cwd, resolved)}`);
  }

  const parsed = parseJsonFile<Manifest>(resolved);
  if (parsed.version !== 1) {
    throw new ContextError("AICTX_CONFIG_INVALID", "Manifest version must be 1");
  }
  ...
  if (!parsed.targets || typeof parsed.targets !== "object") {
    throw new ContextError("AICTX_CONFIG_INVALID", "Manifest targets must be an object");
  }

  const targets = Object.entries(parsed.targets);
  if (!targets.find(([name]) => name === "root")) {
    throw new ContextError("AICTX_CONFIG_INVALID", "Manifest targets must define root");
  }
...
function validateScope(scope: ScopeDefinition, index: number): ScopeDefinition {
  ...
  if (scope.parity !== undefined && typeof scope.parity !== "boolean") {
    throw new ContextError("AICTX_CONFIG_INVALID", `Scope '${id}' parity must be boolean`);
  }
...
export function loadScopeManifest(cwd: string, manifest: Manifest): ScopeManifest {
  ...
  if (!exists(resolved)) {
    throw new ContextError(
      ...
      `Missing scope manifest: ${rel(cwd, resolved)}`
    );
  }

  const parsed = parseJsonFile<ScopeManifest>(resolved);
  if (parsed.version !== 1) {
    throw new ContextError("AICTX_CONFIG_INVALID", "Scope manifest version must be 1");
  }
  if (!Array.isArray(parsed.scopes)) {
    throw new ContextError("AICTX_CONFIG_INVALID", "Scope manifest scopes must be an array");
  }
  if (parsed.claudeRulesDir !== undefined && typeof parsed.claudeRulesDir !== "string") {
    throw new ContextError(
      "AICTX_CONFIG_INVALID",
      "Scope manifest claudeRulesDir must be a non-empty string when provided"
    );
  }
  if (parsed.claudeRulesDir !== undefined && parsed.claudeRulesDir.trim().length === 0) {
    throw new ContextError(
      "AICTX_CONFIG_INVALID",
      "Scope manifest claudeRulesDir must be a non-empty string when provided"
    );
  ...
  const scopes = parsed.scopes.map((scope, index) => {
    ...
    if (ids.has(normalized.id)) {
      throw new ContextError("AICTX_CONFIG_INVALID", `Duplicate scope id '${normalized.id}'`);
    }
...
function asTargets(value: unknown, sourcePath: string): string[] {
  ...
  throw new ContextError(
    "AICTX_CONFIG_INVALID",
    `Module '${sourcePath}' must define targets as string or string[]`
  );
}
...
export function loadModules(cwd: string, manifest: Manifest): ContextModule[] {
  ...
  if (!exists(modulesDir)) {
    throw new ContextError(
      "AICTX_CONFIG_MISSING",
      `Missing modules directory: ${rel(cwd, modulesDir)}`
    );
  }
  ...
  for (const file of files) {
    ...
    if (seenIds.has(id)) {
      throw new ContextError("AICTX_CONFIG_INVALID", `Duplicate module id '${id}'`);
    }
    seenIds.add(id);
    ...
    if (orderValue !== undefined) {
      if (typeof orderValue !== "number" || !Number.isFinite(orderValue)) {
        throw new ContextError(
          "AICTX_CONFIG_INVALID",
          `Module '${file}' order must be a finite number when provided`
        );
      }
      order = orderValue;
    }
...
export function validateScopeWiring(
  ...
  for (const scope of scopeManifest.scopes) {
    ...
    if (allIncludes.size === 0) {
      throw new ContextError("AICTX_CONFIG_INVALID", `Scope '${scope.id}' has no includes`);
    }
    ...
    for (const includePath of allIncludes) {
      ...
      if (!exists(includeAbs)) {
        throw new ContextError(
          "AICTX_CONFIG_INVALID",
          `Scope '${scope.id}' include does not exist: ${toPosix(includePath)}`
        );
      }
    }
    ...
    if (scope.codexTarget) {
      if (scope.codexTarget === "root") {
        throw new ContextError(
          "AICTX_CONFIG_INVALID",
          `Scope '${scope.id}' cannot map codexTarget to root`
        );
      }
      if (!targetNames.has(scope.codexTarget)) {
        throw new ContextError(
          "AICTX_CONFIG_INVALID",
          `Scope '${scope.id}' codexTarget '${scope.codexTarget}' not in manifest targets`
        );
      }
      if (seenTargets.has(scope.codexTarget)) {
        throw new ContextError(
          "AICTX_CONFIG_INVALID",
          `Duplicate codexTarget mapping for '${scope.codexTarget}'`
        );
      }
    ...
    for (const codexPath of scope.codexAgents ?? []) {
      ...
      if (seenOutputs.has(norm)) {
        throw new ContextError("AICTX_CONFIG_INVALID", `Duplicate generated output path '${norm}'`);
      }
    ...
    for (const claudePath of scope.claudeMemories ?? []) {
      ...
      if (!norm.endsWith("/CLAUDE.md") && norm !== "CLAUDE.md") {
        throw new ContextError(
          "AICTX_CONFIG_INVALID",
          `Scope '${scope.id}' claudeMemories must point to CLAUDE.md files`
        );
      }
    }
    ...
    if (scope.claudeRuleFile) {
      if (!scopeManifest.claudeRulesDir) {
        throw new ContextError(
          "AICTX_CONFIG_INVALID",
          `Scope '${scope.id}' defines claudeRuleFile but scope manifest has no claudeRulesDir`
        );
      }
      if (!scope.claudePaths || scope.claudePaths.length === 0) {
        throw new ContextError(
          "AICTX_CONFIG_INVALID",
          `Scope '${scope.id}' claudeRuleFile requires claudePaths`
        );
      }
      ...
      if (seenOutputs.has(outputPath)) {
        throw new ContextError(
          "AICTX_CONFIG_INVALID",
          `Duplicate generated output path '${outputPath}'`
        );
      }
    ...
    if (scope.claudeRuleFile && scope.claudeMemories && scope.claudeMemories.length > 0) {
      throw new ContextError(
        "AICTX_CONFIG_INVALID",
        `Scope '${scope.id}' cannot define both claudeRuleFile and claudeMemories`
      );
    }
  ...
  for (const [target, outputPath] of Object.entries(manifest.targets)) {
    ...
    if (!seenOutputs.has(norm)) {
      throw new ContextError(
        "AICTX_CONFIG_INVALID",
        `Manifest target '${target}' output '${norm}' is not generated by any scope`
      );
    }
  }
