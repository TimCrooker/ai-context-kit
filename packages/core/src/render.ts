...
function withGeneratedHeader(source: string, body: string): string {
  ...
  return `${header}${normalizeNewlines(body).trim()}\n`;
}
...
function includeBlocks(cwd: string, includePaths: string[]): string[] {
  return includePaths.map((includePath) => {
    ...
    const body = normalizeNewlines(rewriteInlineDocRefs(readUtf8(absolute), includePath)).trim();
    if (!body) {
      throw new ContextError("AICTX_GENERATION_INVALID", `Include file is empty: ${includePath}`);
    }
...
function buildClaudeRule(cwd: string, scope: ScopeDefinition, outputPath: string): string {
  ...
  lines.push("  Source: .ai/context/scopes.json");
  lines.push("  Build: ai-context build");
  lines.push("-->");
  lines.push("");
...
export function collectGeneratedOutputs(
  ...
  function add(outputPath: string, content: string, source: string): void {
    ...
    if (seen.has(norm)) {
      throw new ContextError(
        ...
        `Duplicate output path '${norm}' from ${source}`
      );
    }
    seen.add(norm);
    out.push({ path: norm, content: normalizeNewlines(content), source });
  }
  ...
  for (const scope of scopeManifest.scopes) {
    ...
    if (scope.claudeRuleFile) {
      ...
      if (!rulesDir) {
        throw new ContextError(
          "AICTX_GENERATION_INVALID",
          `Scope '${scope.id}' requested claudeRuleFile but claudeRulesDir is not defined`
        );
      }
  ...
  for (const [target, outputPath] of Object.entries(manifest.targets)) {
    ...
    if (!seen.has(norm)) {
      throw new ContextError(
        "AICTX_GENERATION_INVALID",
        `Manifest target '${target}' output '${norm}' is not generated by scopes`
      );
    }
  }
